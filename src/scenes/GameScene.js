import { Terrain, LEVEL_CONFIGS } from '../modules/Terrain.js';
import { CarPhysics } from '../modules/CarPhysics.js';
...
export class GameScene extends Phaser.Scene {
  ...
  showPauseMenu() {
    ...
    const createBtn = (label, y, color, onClick) => {
      ...
      btn.add([bg, text]);
      btn.setInteractive(new Phaser.Geom.Rectangle(-100, -22, 200, 44), Phaser.Geom.Rectangle.Contains);
    ...
    this.resumeBtn = createBtn('▶ 继续游戏', height / 2 - 20, 0x4caf50, () => this.togglePause());
    this.restartBtn = createBtn('🔄 重新开始', height / 2 + 35, 0x2196f3, () => {
      this.cleanup();
      this.scene.restart({ level: this.level });
    });
    this.menuBtn = createBtn('🏠 返回菜单', height / 2 + 90, 0x9e9e9e, () => {
      this.cleanup();
  ...
  showGameOver(win, message) {
    ...
    this.time.delayedCall(600, () => {
      this.scene.start('GameOverScene', {
        ...
        time: this.scoreManager.formatTime(this.scoreManager.getElapsedTime()),
        health: Math.floor(this.scoreManager.getHealthPercent() * 100),
