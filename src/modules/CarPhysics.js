export class CarPhysics {
  ...
  update(deltaTime, terrain, input) {
    ...
    for (const wheel of wheels) {
      ...
      if (penetration > 0) {
        ...
        const dampingForce = this.suspensionVelFL * this.suspensionDamping;
        wheel.suspensionOffset += springForce - dampingForce;
        ...
        const slipVel = this.vx * Math.cos(terrainAngle) - this.vy * Math.sin(terrainAngle);
  ...
  updateTrail(terrain) {
    ...
    const now = Date.now();
    this.trailPoints = this.trailPoints.filter(p => now - p.time < 3000);
