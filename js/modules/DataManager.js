  ...
  MountainRacer.DataManager = (function() {
    ...
    proto._migrateFromLegacy = function() {
      ...
      var legacyKeys = [
        ...
        { key: 'mountain_racer_unlocked_cars', handler: function(key) {
            var val = localStorage.getItem(key);
            if (val) {
              try {
                var cars = JSON.parse(val);
                if (Array.isArray(cars) && cars.length > 0) {
                  self._data.unlocks.cars = cars;
                }
              } catch (e) {}
            }
          }},
        { key: 'mountain_racer_selected_car', handler: function(key) {
            var val = localStorage.getItem(key);
            if (val) {
              self._data.player.selectedCar = val;
            }
          }},
